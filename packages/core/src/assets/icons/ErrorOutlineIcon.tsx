import React from 'react';

export interface ErrorOutlineIconProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
}

const ErrorOutlineIcon = ({ size, ...props }: ErrorOutlineIconProps) => {
  return (
    <svg
      width={size || '1em'}
      height={size || '1em'}
      viewBox="0 0 25 25"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        d="M11.4583 15.625H13.5416V17.7084H11.4583V15.625ZM11.4583 7.29171H13.5416V13.5417H11.4583V7.29171ZM12.4895 2.08337C6.7395 2.08337 2.08325 6.75004 2.08325 12.5C2.08325 18.25 6.7395 22.9167 12.4895 22.9167C18.2499 22.9167 22.9166 18.25 22.9166 12.5C22.9166 6.75004 18.2499 2.08337 12.4895 2.08337ZM12.4999 20.8334C7.89575 20.8334 4.16659 17.1042 4.16659 12.5C4.16659 7.89587 7.89575 4.16671 12.4999 4.16671C17.1041 4.16671 20.8333 7.89587 20.8333 12.5C20.8333 17.1042 17.1041 20.8334 12.4999 20.8334Z"
        fill="currentColor"
      />
    </svg>
  );
};

export default ErrorOutlineIcon;
